...
# cvt decoder
decoder_args=dict(
        dim=encoder_args['dim'],          
        blocks=encoder_args['bev_embedding']['decoder_blocks'],
        residual=True,
        factor=2
)
...
model = dict(
    ...
    pretrained=None,
    backbone=dict(type="CrossViewTransformerBackbone", 
        dim_last=64,
        dim_out=32,
...
# dataset settings
dataset_type = "NuScenesCameraDataset"
nsweeps = 10
...
train_pipeline = [
    ...
    dict(type="Reformat"),
    dict(type="LoadDataTransform", dataset_dir=dataset_dir, image_config=data['image'])
]
test_pipeline = [
    dict(type="LoadPointCloudFromFile", dataset=dataset_type),
    dict(type="LoadPointCloudAnnotations", with_bbox=True),
